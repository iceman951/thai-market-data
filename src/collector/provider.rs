...
// Each fetch returns the exact source JSON, before any normalization.
pub trait MarketDataProvider {
    fn name(&self) -> &'static str;
    async fn fetch_security_stat(&self, date: &str) -> Result<String, String>;
    async fn fetch_index_stat(&self, date: &str) -> Result<String, String>;
    async fn fetch_market_stat(&self, date: &str) -> Result<String, String>;
    async fn fetch_investor_stat(&self, date: &str) -> Result<String, String>;
    async fn fetch_security_update(&self, date: &str) -> Result<String, String>;
    async fn fetch_corporate_actions(&self, date: &str) -> Result<String, String>;

    async fn fetch(&self, dataset: Dataset, date: &str) -> Result<String, String> {
        match dataset {
            Dataset::SecurityStat => self.fetch_security_stat(date).await,
            Dataset::IndexStat => self.fetch_index_stat(date).await,
            Dataset::MarketStat => self.fetch_market_stat(date).await,
            Dataset::InvestorStat => self.fetch_investor_stat(date).await,
            Dataset::SecurityUpdate => self.fetch_security_update(date).await,
            Dataset::CorporateAction => self.fetch_corporate_actions(date).await,
        }
    }
}
